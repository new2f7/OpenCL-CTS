...
int test_simple_compile_with_callback(cl_device_id deviceID, cl_context context, cl_command_queue queue, int num_elements)
{
    ...
    error = clCompileProgram(program, 1, &deviceID, NULL, 0, NULL, NULL, simple_compile_callback, (void*)once_upon_a_midnight_dreary);
...
static void CL_CALLBACK simple_link_callback(cl_program program, void* user_data)
{
	...
	if (strcmp(when_i_pondered_weak_and_weary, (const char*)user_data) != 0)
	{
		log_error("ERROR: in the simple_compile_callback: Expected '%s' and got %s! (in %s:%d)\n", when_i_pondered_weak_and_weary, (const char*)user_data, __FILE__, __LINE__);
	}
}
...
int test_simple_link_with_callback(cl_device_id deviceID, cl_context context, cl_command_queue queue, int num_elements)
{
	...
	clLinkProgram(context, 1, &deviceID, NULL, 1, &program, simple_link_callback, (void*)when_i_pondered_weak_and_weary, &error);
    test_error( error, "Unable to link a simple program" );
...
int test_simple_library_with_callback(cl_device_id deviceID, cl_context context, cl_command_queue queue, int num_elements)
{
	...
	cl_program my_newly_minted_library = clLinkProgram(context, 1, &deviceID, "-create-library", 1, &program, 
		simple_link_callback, (void*)when_i_pondered_weak_and_weary, &error);
    test_error( error, "Unable to create a simple library" );
...
int test_program_binary_type(cl_device_id deviceID, cl_context context, cl_command_queue queue, int num_elements)
{
...
#if 0
	error = clGetProgramInfo( program, CL_PROGRAM_BINARY_TYPES, sizeof(cl_program_binary_type), &program_type, NULL );
	test_error( error, "Unable to get program binary type" );
	if (program_type != CL_PROGRAM_BINARY_TYPE_SINGLE_COMPILED_OBJECT)
	{
		log_error( "ERROR: Expected program type of a just compiled program to be CL_PROGRAM_BINARY_TYPE_SINGLE_COMPILED_OBJECT (in %s:%d)\n", __FILE__, __LINE__ );
		return -1;
	}
	program_type = -1;
#endif
...
#if 0
		error = clGetProgramInfo( program_with_binary, CL_PROGRAM_BINARIES, sizeof(cl_program_binary_type), &program_type, NULL );
		test_error( error, "Unable to get program binary type" );
		if (program_type != CL_PROGRAM_BINARY_TYPE_SINGLE_COMPILED_OBJECT)
		{
			log_error( "ERROR: Expected program type of a program created from compiled object to be CL_PROGRAM_BINARY_TYPE_SINGLE_COMPILED_OBJECT (in %s:%d)\n", __FILE__, __LINE__ );
			return -1;
		}
		program_type = -1;
#endif
    ...
    test_error( error, "Unable to create a simple library" );
#if 0
    error = clGetProgramInfo( my_newly_minted_library, CL_PROGRAM_BINARY_TYPES, sizeof(cl_program_binary_type), &program_type, NULL );
	test_error( error, "Unable to get program binary type" );
	...
	test_error( error, "Unable to create program with binary" );
#if 0
    error = clGetProgramInfo( library_with_binary, CL_PROGRAM_BINARY_TYPES, sizeof(cl_program_binary_type), &program_type, NULL );
	test_error( error, "Unable to get program binary type" );
	if (program_type != CL_PROGRAM_BINARY_TYPE_LIBRARY)
	{
...
#if 0
  error = clGetProgramInfo( fully_linked_program, CL_PROGRAM_BINARY_TYPES, sizeof(cl_program_binary_type), &program_type, NULL );
	test_error( error, "Unable to get program binary type" );
...
#if 0
		error = clGetProgramInfo( fully_linked_program_with_binary, CL_PROGRAM_BINARY_TYPES, sizeof(cl_program_binary_type), &program_type, NULL );
		test_error( error, "Unable to get program binary type" );
		if (program_type != CL_PROGRAM_BINARY_TYPE_EXECUTABLE)
		{
...
int test_large_compile_and_link_status_options_log(cl_context context, cl_device_id deviceID, cl_command_queue queue, unsigned int numLines)
{
	...
	if (error != CL_SUCCESS || library_log[0]=='\0'){
	...
	if (error != CL_SUCCESS || linking_log[0]=='\0'){
